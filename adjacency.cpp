...
*
*******************************************************************************/
Adjacency::Adjacency(IN int size):pList(NULL),nCapacity(size)
{
        nCapacity = size ;
        Malloc(size) ;
}
...
*
*******************************************************************************/
int Adjacency::Malloc(IN int size)
{
    unsigned int i(0) ;
    if (size > 1)
    {
...
*
*******************************************************************************/
int Adjacency::Realloc(IN int size)
{
        unsigned int i(0) ;
        unsigned oldSize(nCapacity) ;
        nCapacity = static_cast<int>(size * 1.5) ;
        aList ** tmp = new aList * [nCapacity] ;
        ...
        memset(tmp, 0, sizeof(aList *) * nCapacity) ;
...
*
*******************************************************************************/
aList & Adjacency::operator [] (IN unsigned int index)
{
        // 判断是否已经存在
...
*
*******************************************************************************/
unsigned int Adjacency::GetSize(IN unsigned int index)
{
...
*
*******************************************************************************/
int Adjacency::ImportFile(IN const char *szFileName)
{
    if (szFileName == NULL)
    {
        return 0 ;
...
*
*******************************************************************************/
int Adjacency::ExportFile(IN const char *szFileName)
{
        if (szFileName == NULL)
        {
                return 0 ;
...
int Adjacency::Clear()
{
        ...
        for (unsigned int i = 0; i < nCapacity; ++i)
        {
                aList * p = pList[i] ;
                p->Clear() ;
        }
